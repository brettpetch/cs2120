...
def compute_average_change(temperatures):
    ...
    """
    avg_change = round(sum([c - temperatures[i - 1] for i, c in enumerate(temperatures)][1:]) / (len(temperatures) - 1),
                       2)
    return avg_change
...
def compute_average_changes_dict(station_info_dict,
    ...
    for station in station_names:  # Iterate over stations
        ...
        for year in np.arange(start_year, end_year, 1):  # Iterate over years from start to end (end is exclusive)
            temperatures = (get_temperatures_for_year(station, year, station_name_dict,
                                                      temperature_data_dict))  # append temperature of lists in parent list
        ...
        avg_change_dict[station] = avg_period_change
        # Insert the correct key (station name) and average change value into the dictionary
...
def draw_maps_by_range(station_info_dict, 
    ...
    while (i < n):  # Condition to loop over
        ...
        draw_avg_change_map(station_names, station_name_dict, station_info_dict, temperature_data_dict, start_year,
                            end_year)
