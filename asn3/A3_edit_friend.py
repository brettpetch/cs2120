...
def compute_average_changes_dict(station_info_dict,
    ...
    """
    dicts_average_changes = {}
    for key, value in temperature_data_dict.items():
        list_of_average_year = []
        for year, data in value.items():
            if start_year <= year < end_year:
                mean_average_of_year = compute_average_temp(data)
                list_of_average_year.append(mean_average_of_year)
        dicts_average_changes[key] = compute_average_change(list_of_average_year)
    return dicts_average_changes
