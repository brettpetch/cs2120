...
#temp_data_dict = make_valid_temperature_data_dict(station_info_dict, temp_data_dict, 1950, 2000)
...
def compute_average_change(temperatures):
    ...
    while n < len(temperatures) - 1: 
        templist = templist + [temperatures[n+1] - temperatures[n]]  # keeps adding temperatures to templist while
                                                                     # making sure to not add redundancies
        n = n+1  # increment the counter
    return np.mean(templist)  # give mean of items in list
...
def compute_average_changes_dict(station_info_dict,
    ...
    average_change_dict = dict()
    for station_id in station_info_dict:
        templist = []
        if station_id in temperature_data_dict:
            for year in range(start_year, end_year):
                templist = templist + [compute_average_temp(temperature_data_dict[station_id][year])]
            average_change_dict[station_id] = compute_average_change(templist)
    return average_change_dict

#avg_changes_dict = compute_average_changes_dict(station_info_dict, temp_data_dict, 1950, 2000)
#avg_changes_dict = compute_average_changes_dict(station_info_dict, temp_data_dict, 1980, 2000)
...
def compute_top_average_change_dict(average_changes_dict, n):
    ...
    :return: For convenience, a reference to the reduced input dictionary 
     ...
     temperature changes.
    """
    top_average_change_dict = dict()
    i = 0
    for (key, value) in sort_dictionary_by_absolute_value_descending(average_changes_dict):
        if i < n:
            top_average_change_dict[key] = value
            i = i + 1

    return top_average_change_dict
    

#top = compute_top_average_change_dict(avg_changes_dict, 4)
...
def make_average_change_dict_for_map(station_info_dict, average_change_dict):
    ...
    """
    data_tuple = dict()
    for station_id in average_change_dict.keys():
        data_tuple[station_info_dict[station_id]['station_name']] = (
        station_info_dict[station_id]['lat'], station_info_dict[station_id]['lon'], average_change_dict[station_id])
    return data_tuple
    

#formap = make_average_change_dict_for_map(station_info_dict, avg_changes_dict)
#topformap = make_average_change_dict_for_map(station_info_dict, top)
...
def draw_top_average_changes_map(top_average_changes_dict_for_map,
    ...
    draw_map('Top ' + str(num_top_values) + " Average Annual Temperature Changes Between " + str(start_year) + " to " + str(end_year - 1) + ".", top_average_changes_dict_for_map)
...
def draw_maps_by_range(station_info_dict, 
    ...
    for i in range(num_maps):
        end_year = start_year + years_per_map #Defines the end year based on the parameters 
        
        #Create the average temperature changes dictionary, and use that to create the top average temperature changes dictionary
        avg = compute_average_changes_dict(station_info_dict, valid_temperature_data_dict, start_year, end_year)
        topavg = compute_top_average_change_dict(avg, num_top_values)
        
        #Create the data_dict which is a tuple required for plotting on the map
        data_dict = make_average_change_dict_for_map(station_info_dict, topavg)
        
        #Using the data_dict that was created for topavg, draw the maps
        draw_top_average_changes_map(data_dict, start_year, end_year, num_top_values)
        
        #Sets the new starting position of the start_year
        start_year = end_year
...
temperature_data = make_valid_temperature_data_dict(station_data, temperature_data, start_year, end_year)
...
num_top_values = 5
num_maps = 2
draw_maps_by_range(station_data, temperature_data, start_year, years_per_map, num_top_values, num_maps)
